...
public class DriveSystem extends Subsystem implements DebugOutput.Debugable {
	...
	//positionMotors[0].setPulseWidthPosition(/* positionMotors[0].getPulseWidthPosition() + */ 4096);
	//positionMotors[1].setPulseWidthPosition(/* positionMotors[1].getPulseWidthPosition() + */ 4096);
	//positionMotors[2].setPulseWidthPosition(0);
	//positionMotors[3].setPulseWidthPosition(/* positionMotors[3].getPulseWidthPosition()  + */ 4096);
	
	
	positionMotors[0].reverseOutput(false);
	positionMotors[1].reverseOutput(false);
	positionMotors[2].reverseOutput(false);
	positionMotors[3].reverseOutput(true);

	positionMotors[0].reverseSensor(true);
	positionMotors[1].reverseSensor(true);
	positionMotors[2].reverseSensor(true);
	positionMotors[3].reverseSensor(true);

	for (int i = 0; i < positionMotors.length; i++) {
	    /*
	     * Makes accurate movement simpler if the codes per revolution is
	     * accurate.
	     */
	    positionMotors[i].configEncoderCodesPerRev(4096 * 2);

	    // positionMotors[i].setEnRobotMapcPosition(0);

	    /* set the peak and nominal outputs, 12V means full */
	    positionMotors[i].configPeakOutputVoltage(+12f, -12f);
	    positionMotors[i].configNominalOutputVoltage(+3.0, -3.0);

	    /*
	     * V set the allowable closed-loop error, Closed-Loop output will be
	...
	/*
	 * Integer pulseWidthPos = new
	 * Integer(positionMotors[0].getPulseWidthPosition()); output[0] =
	 * "Talon Mag encoder FR Pulse width position: " +
	 * pulseWidthPos.toString() + "\n";
	 * 
	 * Integer pulseWidthUs = new
	 * Integer(positionMotors[0].getPulseWidthRiseToFallUs()); output[1] =
	 * "Talon Mag encoder FR Pulse width microseconds: " +
	 * pulseWidthUs.toString() + "\n";
	 * 
	 * Integer periodUs = new
	 * Integer(positionMotors[0].getPulseWidthRiseToRiseUs()); output[2] =
	 * "Talon Mag encoder FR period: " + periodUs.toString() + "\n";
	 * 
	 * Integer pulseWidthVel = new
	 * Integer(positionMotors[0].getPulseWidthVelocity()); output[3] =
	 * "Talon Mag encoder FR pulse width velocity: " +
	 * pulseWidthVel.toString() + "\n";
	 * 
	 * FeedbackDeviceStatus sensorStatus =
	 * positionMotors[0].isSensorPresent(FeedbackDevice.
	 * CtreMagEncoder_Absolute); Boolean sensorPluggedIn = new
	 * Boolean(FeedbackDeviceStatus.FeedbackStatusPresent == sensorStatus);
	 * output[4] = "Talon Mag encoder FR is sensor plugged in: " +
	 * sensorPluggedIn.toString() + "\n";
	 */
