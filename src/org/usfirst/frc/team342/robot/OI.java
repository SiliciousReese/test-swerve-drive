...
public class OI {
	 ...
	 * button on a joystick. You create one by telling it which joystick it's on
	 * and which button number it is. Joystick stick = new Joystick(port);
	...
	private static final int DEBUG_BUTTON = 1;

	public static void setupButtons(DebugOutput.Debugable debugables[]) {
		Button debug = new JoystickButton(new Joystick(JOY_NUM), DEBUG_BUTTON);

		debug.whileHeld(new DebugOutput(debugables));
	}
}
